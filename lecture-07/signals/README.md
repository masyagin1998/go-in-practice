...
| **IPC** — события от другого процесса | SIGUSR1/SIGUSR2, custom | `signalfd/`, `thread-masking/` |
...
thread-masking/      # маска сигналов на уровне треда; кто ловит и кто нет
```
...
- **Маска — per-thread**. `kill(pid, sig)` доставляется ЛЮБОМУ треду
  с разблокированным сигналом. Если нужно направить сигнал в
  конкретное место — блокируйте на всех остальных (пример в
  `thread-masking/`).
